...
(function() {
  ...
  ['dragenter', 'dragover'].forEach(function(ev) {
    dropzone.addEventListener(ev, function(e) { e.preventDefault(); e.stopPropagation(); dropzone.classList.add('eo-dragover'); });
  });
  ['dragleave', 'drop'].forEach(function(ev) {
    dropzone.addEventListener(ev, function(e) { e.preventDefault(); e.stopPropagation(); dropzone.classList.remove('eo-dragover'); });
  });
...
function eoBatchStatusBadge(status) {
  var map = {
    pending:    { cls: 'badge-status0', icon: 'fa-clock',                label: eoBatchI18n.statusPending },
    processing: { cls: 'badge-status1', icon: 'fa-sync fa-spin',         label: eoBatchI18n.statusProcessing },
    completed:  { cls: 'badge-status4', icon: 'fa-check',                label: eoBatchI18n.statusCompleted },
    partial:    { cls: 'badge-status3', icon: 'fa-exclamation-triangle',  label: eoBatchI18n.statusPartial },
    failed:     { cls: 'badge-status8', icon: 'fa-times',                label: eoBatchI18n.statusFailed },
    cancelled:  { cls: 'badge-status9', icon: 'fa-ban',                  label: eoBatchI18n.statusCancelled }
  };
  var s = map[status] || { cls: 'badge-status0', icon: 'fa-question', label: status };
...
function eoBatchLoadList(page) {
  ...
  var ajaxData = { action: 'batchList', page: page, per_page: perPage, status: statusFilter };
...
function eoBatchShowDetail(uuid) {
  ...
  jQuery.ajax({
    url: eoBatchAjaxUrl,
    type: 'POST',
    data: { action: 'batchStatus', uuid: uuid },
    dataType: 'json',
    success: function(res) {
      if (res.status !== 'ok') {
...
function eoBatchLoadResults(uuid) {
  ...
  jQuery.ajax({
    url: eoBatchAjaxUrl,
    type: 'POST',
    data: { action: 'batchResults', uuid: uuid },
    dataType: 'json',
    success: function(res) {
      if (res.status !== 'ok') {
      ...
      for (var i = 0; i < docs.length; i++) {
        ...
        if (dDocNum && dStatus === 'completed') {
          ...
          if (!_invoiceCheckQueue[dDocNum]) {
            _invoiceCheckQueue[dDocNum] = { taxId: dTaxId, data: dData, rows: [] };
          }
      ...
      for (var docNum in _invoiceCheckQueue) {
        ...
        (function(dn, entry) {
          setTimeout(function() { eoBatchRowInvoiceCheck(dn, entry.taxId, entry.data, entry.rows); }, 200 + _checkDelay);
  ...
  if (Array.isArray(items) && items.length > 0) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (it.taxes && Array.isArray(it.taxes) && it.taxes.length > 0) {
        taxStr = it.taxes.map(function(t) { return (t.tax_rate || 0) + '%'; }).join(', ');
      }
      var typeIcon = '';
...
function eoBatchCheckAndRenderInvoiceButton(docNum, docNumSafe, taxId, data) {
  ...
  if (!statusEl || !actionEl) { console.log('[EasyOCR]   → elements not found, skipping'); return; }
  ...
  if (row.style.display === 'none') {
    ...
    if (statusEl && !statusEl.dataset.checked) {
      ...
      if (statusEl.dataset.docDataB64) {
        try { docData = JSON.parse(decodeURIComponent(escape(atob(statusEl.dataset.docDataB64)))); } catch(e) {}
      }
      if (docNum) {
